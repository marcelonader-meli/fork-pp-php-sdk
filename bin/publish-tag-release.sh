...
if [ -n "${SSH_SDK_DEPLOY_KEY_TEST:=}" ]; then
  echo "[+] Using SSH_SDK_DEPLOY_KEY_TEST"

  mkdir -p "$HOME/.ssh"
  DEPLOY_KEY_FILE="$HOME/.ssh/deploy_key"
  echo "${SSH_SDK_DEPLOY_KEY_TEST}" >"$DEPLOY_KEY_FILE"
  ...
  echo "::error::SSH_SDK_DEPLOY_KEY_TEST is empty."
  exit 1
fi

cd "$SOURCE_DIRECTORY"
### Preparing release folder
rm -rf .git .github .gitmodules .fury bin/publish-tag-release.sh
